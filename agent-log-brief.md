...
| Date | Action |
|------|--------|
| 2026-04-08 | Added footer bar with last commit date and GitHub link (commit c915978): Footer displays on both index.html and visualization/aam.html; fetches commit date from GitHub API (api.github.com/repos/itpyi/AtomISA/commits/main); caches result in localStorage for 1 hour; includes "View on GitHub" link; styled to match dark header theme; graceful error handling. |
| 2026-04-08 | Separated preset and download data structures by mode (commit 4a3b6a1): Added mode property to presets; presets dropdown now filters by current mode (Original shows 2 presets, QN shows 1); saveToFile() saves mode-specific JSON (Original: motion matrix, QN: ax, ay, x1[], y1[]); applyParamData() auto-switches mode based on JSON mode field; maintained backward compatibility. |
